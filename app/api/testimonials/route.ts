...
const FILE = path.join(process.cwd(), 'public', 'testimonials', 'testimonials.json');
...
export async function POST(request: NextRequest) {
    try {
        ...
        const data = getData();
        const item = { id: Date.now(), name: body.name || '', company: body.company || '', comment: body.comment || '', avatar: body.avatar || '', rating: body.rating || 5, active: true };
        data.push(item);
        saveData(data);
