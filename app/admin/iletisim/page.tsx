...
export default function IletisimAdmin() {
        ...
        fetch('/api/db/settings?key=contact', { cache: 'no-store' })
            ...
            .then(d => {
                if (d && Object.keys(d).length > 0) setData(d);
                    ...
                    fetch('/api/contact', { cache: 'no-store' }).then(r => r.json()).then(old => {
                        setData(old);
    ...
    const save = async () => {
        const res = await fetch('/api/db/settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: 'contact', value: data }) });
        ...
        <div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white border border-neutral-200 p-6 space-y-4 rounded-lg">
                    ...
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">Email</label><input value={data.info?.email || ''} onChange={e => setData({ ...data, info: { ...data.info, email: e.target.value } })} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">Telefon</label><input value={data.info?.phone || ''} onChange={e => setData({ ...data, info: { ...data.info, phone: e.target.value } })} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">Adres</label><textarea value={data.info?.address || ''} onChange={e => setData({ ...data, info: { ...data.info, address: e.target.value } })} rows={2} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">Çalışma Saatleri</label><input value={data.info?.hours || ''} onChange={e => setData({ ...data, info: { ...data.info, hours: e.target.value } })} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                </div>

                {/* Social */}
                <div className="bg-white border border-neutral-200 p-6 space-y-4 rounded-lg">
                    <h2 className="font-bold text-lg">Sosyal Medya & WhatsApp</h2>
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">WhatsApp Numarası</label><input value={data.whatsapp?.number || ''} onChange={e => setData({ ...data, whatsapp: { ...data.whatsapp, number: e.target.value } })} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">WhatsApp Mesajı</label><input value={data.whatsapp?.message || ''} onChange={e => setData({ ...data, whatsapp: { ...data.whatsapp, message: e.target.value } })} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                    {(data.social || []).map((s: { platform: string; url: string }, i: number) => (
                        <div key={i} className="grid grid-cols-2 gap-4">
                            <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">Platform</label><input value={s.platform} onChange={e => { const soc = [...data.social]; soc[i] = { ...soc[i], platform: e.target.value }; setData({ ...data, social: soc }); }} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                            <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">URL</label><input value={s.url} onChange={e => { const soc = [...data.social]; soc[i] = { ...soc[i], url: e.target.value }; setData({ ...data, social: soc }); }} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-white border border-neutral-200 p-6 space-y-4 rounded-lg">
                    ...
                    <div><label className="block text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-1">Google Maps Embed URL</label><input value={data.map?.embedUrl || ''} onChange={e => setData({ ...data, map: { ...data.map, embedUrl: e.target.value } })} className="w-full border border-neutral-200 px-3 py-2 text-sm focus:outline-none focus:border-black rounded" /></div>
                </div>
            </div>
